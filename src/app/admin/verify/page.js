    ...
    <div className="min-h-screen bg-background p-6 md:pl-56 transition-colors duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <h1 className="text-2xl font-black text-on-surface tracking-tight mt-1.5">
      ...
      {loading ? (
        ...
        <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-12 text-center max-w-lg mx-auto shadow-sm space-y-4 transition-colors duration-300">
          <div className="bg-teal-500/10 text-primary p-3 rounded-full w-fit mx-auto">
            <ShieldCheck className="w-8 h-8" />
          </div>
          <div>
            <h3 className="font-bold text-on-surface text-lg">No profiles in this filter!</h3>
            <p className="text-xs text-on-surface-variant mt-1">There are currently no user profiles matching &quot;{activeFilter}&quot;.</p>
          </div>
          <div className="flex justify-center gap-2 pt-2">
            <Button onClick={handleAddSampleKYC} variant="primary" className="text-xs font-bold">
              Add Sample KYC Profile
            </Button>
            <Button onClick={handleResetDemoData} variant="secondary" className="text-xs font-bold">
              Reset Demo Users
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 animate-in fade-in duration-300">
          {filteredProfiles.map((p) => {
            ...
            return (
              <Card key={p.id} className="bg-surface-container-lowest border border-outline-variant p-6 flex flex-col justify-between gap-5 shadow-sm hover:shadow-md transition-all duration-300 rounded-2xl">
                
                {/* Profile Meta info */}
                <div className="flex flex-col sm:flex-row gap-5 items-start">
                  {/* NID Card Photo Container */}
                  <div className="relative w-full sm:w-48 h-32 rounded-xl overflow-hidden bg-surface-container-low border border-outline-variant group flex-shrink-0">
                    <img 
                    ...
                    <button 
                      ...
                      className="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity rounded-xl outline-none cursor-pointer"
                    >
                  ...
                  <div className="space-y-2.5 flex-grow">
                    ...
                    <div className="space-y-1.5 text-xs text-on-surface-variant font-medium">
                      ...
                      <p className="flex items-center gap-1.5 font-bold text-on-surface">
                        ...
                        Wallet Balance: <strong className="text-secondary">{parseFloat(p.wallet_balance || 0).toFixed(2)} BDT</strong>
                      </p>
                ...
                {/* Action buttons */}
                <div className="flex flex-col sm:flex-row gap-2 pt-3 border-t border-outline-variant/40">
                  {!isVerified ? (
                    <Button 
                    ...
                    <div className="flex-1 py-2 text-xs font-bold text-emerald-600 bg-emerald-500/10 rounded-lg flex items-center justify-center gap-1">
                  ...
                  {!isSuspended && (
                    <Button 
                      ...
                      variant="danger" 
                      className="flex-1 py-2 text-xs bg-red-500/10 hover:bg-red-500/20 text-red-600 dark:text-red-400 border border-red-500/20 font-bold uppercase tracking-wider"
                    >
      ...
      {selectedPhoto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative max-w-4xl max-h-[85vh] z-10 overflow-hidden rounded-xl border border-outline-variant shadow-2xl">
            <img src={selectedPhoto} alt="NID Zoomed View" className="w-full h-auto max-h-[80vh] object-contain bg-slate-950" />
            <button 
              onClick={() => setSelectedPhoto(null)}
              className="absolute top-4 right-4 bg-slate-950/70 hover:bg-slate-950 text-white rounded-full px-3 py-1.5 transition-colors font-bold text-[10px] uppercase tracking-wider outline-none"
            >
              Close Zoom
            </button>
          </div>
